...
## This programme is in DRAFT

Timings and room scheduling may be subject to change. Links to the conference proceedings will be added once they are up on [zenodo.org](https://zenodo.org/).
:::
...
days <- read_csv("program.csv") |> 
   mutate(
    ...
    Paper=pmap_chr(list(Title, Author, Link),
     ~html(glue("<a href='{..3}' style='font-size:11pt'>{link_fa}</a><span style='font-style:italic; font-size:12pt'> {..1}</span><br><span style='font-style:italic; font-size:11.5pt'>--&nbsp;{..2}</span>"))
    ),
     Paper=case_when(
      !is.na(Track) ~ Track,
      !is.na(Author) ~ Paper,
  ...
  #   across(c(When), ~if_else(is.na(.x), "", .x)),
  #    Paper=pmap_chr(list(Title, Author, Link),
  #    ~html(glue("<a href='{..3}' style='font-size:11pt'>{link_fa}</a><span style='font-style:italic; font-size:11.5pt'> {..1}</span><br><span style='font-style:italic; font-size:11.5pt'>--&nbsp;{..2}</span>"))
  #    ),
  #   #Paper=pmap_chr(list(Title, Author, Link),
  #    #~html(glue("<span style='font-style:italic; font-size:11.5pt'> {..1}</span><br><span style='font-style:italic; font-size:11.5pt'>--&nbsp;{..2}</span>"))
  #   #),
  #    Paper=case_when(
  #     !is.na(Track) ~ Track,
  #     !is.na(Author) ~ Paper,
...
gt(groupname_col = "Type") |> 
  cols_align("left", Type) |>
    ...
    tab_options(data_row.padding = px(.5),
              table.font.size = px(18),
              column_labels.font.size = px(18), table.align="left", row_group.font.size = px(16))  #|>
              #tab_header(title=html("<span style='font-weight:bold; #font-size:20pt'>Tue 9th April</span>"), subtitle = html(fn_text)) |>
```
  ...
  cols_align("left", Type) |>
    ...
    tab_style(
      style = cell_borders(
          ...
          locations = cells_body(rows = c(1,5,7,8,9,11,12,13,16,17,18,20,21,22))
    ) #|>
       #       tab_header(title=html("<span style='font-weight:bold; font-size:20pt'>Wed 10th April</span>"), subtitle = html(fn_text)) |>
```
    ...
    tab_style(
      style = cell_borders(
          ...
          locations = cells_body(rows = c(1,2,3,5,6,7,10,11,12,14,15,16,18,19,20,23,24,25,26,27,29,30,31,33,34,35,38,39,40,42,43,44)) 
             ...
             # tab_header(title=html("<span style='font-weight:bold; #font-size:20pt'>Thu 11th April</span>"), subtitle = html(fn_text))  |>
```
...
gt(groupname_col = "Type") |> 
  ...
  tab_style(
     ...
     # tab_header(title=html("<span style='font-weight:bold; font-size:20pt'>Fri 12th April</span>"), subtitle = html(fn_text)) |>
```